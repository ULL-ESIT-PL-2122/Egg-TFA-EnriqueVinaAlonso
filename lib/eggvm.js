...
function translateToJs(fileName) {
  let env = Object.create(topEnv);

  const template = (declarations, jsCode) => declarations.length ? `var ${declarations.join(', ')};\n${jsCode}` : jsCode;
  try {
  ...
  return tree.generateJS(env);
}

module.exports = {json2AST, run, runFromFile,runFromEVM, topEnv, specialForms, parser, evaluate, parseFromFile, translateToJs};
